...
$is_episode = isset($_GET['t']) && $_GET['t'] == 'ser';
$link = $is_episode
    ? unserialize(get_term_meta(intval(get_query_var('trid')), 'trglinks_' . intval(get_query_var('trembed')), true))
    : unserialize(get_post_meta(intval(get_query_var('trid')), 'trglinks_' . intval(get_query_var('trembed')), true));
...
<head>
    ...
    <title><?php _e('Embed', 'onwatch'); ?></title>
