...
<div class="ow-container">
    <header class="ow-section__header" style="padding-top: 1rem;">
        <h1 class="ow-section__title"><?php _e('المسلسلات', 'onwatch'); ?></h1>
        <span class="ow-archive__count"><?php echo $wp_query->found_posts; ?> <?php _e('مسلسل', 'onwatch'); ?></span>
    </header>
    ...
    <div class="ow-grid" data-archive-grid data-infinite="<?php echo $infinite_scroll ? 'true' : 'false'; ?>">
        <?php if (have_posts()): while (have_posts()): the_post(); get_template_part('resources/views/components/card', 'series'); endwhile; endif; ?>
    </div>

    <?php if (!have_posts()): ?>
    <div class="ow-empty">
        <div class="ow-empty__title"><?php _e('لا توجد مسلسلات', 'onwatch'); ?></div>
        <p class="ow-empty__desc"><?php _e('لم يتم إضافة أي مسلسلات بعد.', 'onwatch'); ?></p>
    </div>
    <?php endif; ?>
